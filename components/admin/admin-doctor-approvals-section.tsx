...
type AdminDoctorApprovalsSectionProps = {
  onToast: (message: string) => void;
};

export function AdminDoctorApprovalsSection({
  onToast,
}: AdminDoctorApprovalsSectionProps) {
  ...
  const loadDoctors = useCallback(async () => {
    ...
    try {
      ...
      if (!res.ok) {
        setLoadState({
          status: "error",
          message: body.details ?? body.error ?? "Could not load pending doctors.",
        });
        return;
      }
  ...
  async function handleApprove(doctor: AdminDoctorListItem) {
    ...
    try {
      ...
      if (!res.ok) {
        onToast(body.details ?? body.error ?? "Approval failed. Please try again.");
        return;
      }

      onToast(`${doctor.full_name} has been approved.`);
      await loadDoctors();
    } catch {
      onToast("Network error while approving. Please try again.");
    } finally {
      setActingId(null);
      setActingType(null);
    }
  ...
  async function handleDeny(doctor: AdminDoctorListItem) {
    ...
    try {
      ...
      if (!res.ok) {
        onToast(body.details ?? body.error ?? "Deny failed. Please try again.");
        return;
      }

      onToast(`${doctor.full_name} has been denied.`);
      await loadDoctors();
    } catch {
      onToast("Network error while denying. Please try again.");
    } finally {
      setActingId(null);
      setActingType(null);
    }
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-slate-600">
            Review registrations waiting for approval. Approved doctors appear in
            the public directory and unlock their full dashboard.
          </p>
                    ...
                    <tr key={doctor.id} className="hover:bg-slate-50/80">
                      ...
                      <td className="whitespace-nowrap px-4 py-4 sm:px-6">
                        <div className="flex flex-wrap gap-2">
                          ...
                          >
                            {actingId === doctor.id && actingType === "approve" ? (
                              <>
                                <Loader2
